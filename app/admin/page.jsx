...
/* --- Main component --- */
export default function ChapterFormatterPages() {
  /* raw & book */
  ...
  function handleFormat() {
    ...
    const enriched = parsed.map((ch) => ({
      ...
      loading: false,
      error: null,
      // progress object stores per-language resume info:
      // progress: { en: { nextStart: 0, text: "" }, es: { ... } }
      progress: {},
    }));
  ...
  // Translates `text` (full optimizedEnglish) into `langs`, using per-chapter progress stored in chapter.progress.
  // It updates chapter.progress in place after each successful partial translation.
  // Returns an array of objects: [{ lang, text, error? }, ...]
  async function translateWithResume(chIndex, text, langs) {
    ...
    // Helper to update progress in state for this chapter
    ...
    // Process languages sequentially (safer for throttling). You can parallelize later if needed.
    for (const lang of langs) {
      appendLog(`Translating ${lang} for chapter ${chIndex + 1} — starting/resuming...`);
      // See if we already have progress for this language
      const existing = (ch.progress && ch.progress[lang]) || null;
      let start = existing?.nextStart ?? 0;
      let accumulated = existing?.text ?? "";

      // If everything already finished, just use accumulated (edge case)
      ...
      // Keep calling the translate endpoint until we've translated all paragraphs or an error occurs.
      // Each call asks server to start at `start` so server will only translate remaining paragraphs (batches).
      let finished = false;
      while (!finished) {
        try {
          // Request translation for remaining paragraphs (server will batch internally)
          const resp = await callTranslateAPI(text, [lang], start);
          // resp.translations may be either:
          // - old shape: [{ lang, text }] (full text)
          // - new resumable shape: [{ lang, text, startParagraph, paragraphCount, completed, error? }]
          ...
          // If server returned simple {lang, text} (no pagination), consider done
          if (typeof info.completed === "undefined" && typeof info.paragraphCount === "undefined") {
            // assume this is the full translated text
          ...
          // append returned chunk
          accumulated = accumulated ? accumulated + "\n\n" + returnedText : returnedText;
          // compute next start (start index for next call)
          ...
          // If there's an error field, stop and return partial result + error
          if (info.error) {
          ...
          // else loop again using nextStart
          start = nextStart;
        } catch (err) {
          // Network or unexpected error. Save partial accumulated progress to state and return partial result with error.
    ...
    try {
      ...
      for (const t of allTranslations) {
        ...
        try {
          ...
          // If enhancement fails for a language, still add a fallback entry with error
          enhancedList.push({
            lang: t.lang,
            text: t.text,
            enhanced: t.text,
      ...
      // clear progress for this chapter (translation finished successfully for those languages)
      setChapters((prev) =>
  ...
  async function generateSpeech(chIndex, lang) {
    ...
    try {
      ...
      setChapters((prev) =>
        prev.map((c, i) =>
          i === chIndex ? { ...c, translations: c.translations.map(tr => tr.lang === lang ? { ...tr, audioUrl: json.url, audioLoading: false, audioError: null } : tr) } : c
        )
      );
                  ...
                  <div className="mt-3 flex flex-col md:flex-row md:items-center md:gap-3">
                    <div className="flex items-center gap-2">
                      ...
                      <select value={t.voice || DEFAULT_VOICES[t.lang] || ""} onChange={(e) => {
                        ...
                        <option value="">{DEFAULT_VOICES[t.lang] || "Default voice"}</option>
